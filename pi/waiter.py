...
class KobukiRobot:
    ...
    def push_button(self):
        if self.state == RobotStatus.LOADING_UNLOADING:
            self.state = RobotStatus.DELIVERING_ORDER
            if len(self.drinks > 0):
                self.next_waypoint = graph.find_route(self.prev_waypoint, self.targets)[1]
            else:
                self.next_waypoint = None
